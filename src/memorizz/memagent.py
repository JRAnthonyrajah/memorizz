...
class MemAgent:
    ...
    async def run_streaming(self, query: str, memory_id: str = None, conversation_id: str = None) -> AsyncIterator[str]:
        ...
        try:
            ...
            # 6) Stream the response using the LLM's async_stream_text method
            # For simplicity, we'll stream a simple response without tool calls
            # (Tool calls require complete responses to parse function arguments)
            full_response = ""
            async for chunk in self.model.async_stream_text(augmented_query, system_prompt):
                full_response += chunk
                yield chunk

            # 7) After streaming is complete, finalize the response
            self._finalize_response(full_response, [], conversation_id, memory_id)

            # 8) Cache the complete response (if semantic cache enabled)
            if self.semantic_cache_instance:
                self.semantic_cache_instance.set(
                    query=query,
