...
class MemAgent:
    ...
    async def run_streaming(self, query: str, memory_id: str = None, conversation_id: str = None) -> AsyncIterator[str]:
        """
        Run the agent with streaming responses and full tool execution support.

        This method runs the agent with the given query and streams the response in chunks
        as they are generated by the LLM. Tool calls are handled synchronously within the loop,
        and only the final text response is streamed.
        ...
        """
        logger.info(f"AGENT STREAMING RUN START: Agent {self.agent_id} executing query: {query[:50]}...")

        try:
            # Check if we're in multi-agent mode
            if self.is_multi_agent_mode:
                # Multi-agent mode doesn't support streaming yet
                logger.warning("Multi-agent mode does not support streaming. Falling back to regular run.")
                result = self.run(query, memory_id, conversation_id)
                yield result
                return
            ...
            # 7) Execute main streaming loop with tool support
            async for chunk in self._execute_streaming_loop(messages, query, memory_id, conversation_id):
                yield chunk

        except Exception as e:
            logger.error(f"Error in streaming run for agent {self.agent_id}: {e}")
            yield f"An error occurred while running the agent: {e}"
    ...
    async def _execute_streaming_loop(
        ...
        """
        Execute the main agent interaction loop with tool calls and streaming responses.

        This is the streaming version of _execute_main_loop. It handles tool execution
        synchronously but streams the final text response.
        ...
        for step in range(self.max_steps):
            ...
            # b) Check if we should stream (only on final response, not during tool calls)
            # Call the LLM API using the configured model
            ...
            if tool_calls:
                ...
                if self.enable_planning:
                    ...
                    try:
                        ...
                        messages = await self._execute_plan_mode(tool_calls, messages, query, memory_id, tool_metas)
                else:
                    # Use traditional serial execution (run in thread to avoid blocking)
                    messages = await asyncio.to_thread(
                        self._handle_tool_calls, tool_calls, messages, query, memory_id, tool_metas
                    )
                continue

            # h) No function calls → stream the final answer
            if response.output_text:
                # Stream the final response
                final_text = response.output_text

                # Finalize response (record to memory, etc.)
                await asyncio.to_thread(
                    self._finalize_response, final_text, messages, conversation_id, memory_id
                )

                # Cache the response (if semantic cache enabled)
                if self.semantic_cache_instance:
                    self.semantic_cache_instance.set(
                        query=query,
                        response=final_text,
                ...
                # Stream the text in chunks (simulate streaming for now, could use actual LLM streaming)
                # For now, yield the complete text, but this could be enhanced with actual streaming
                yield final_text
                return
